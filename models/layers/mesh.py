    ...
    def discrete_project(self, pc: torch.Tensor, thres=0.9, cpu=False):
        with torch.no_grad():
            ...
            if isinstance(self, Mesh):
                ...
                mid_points = self[:, :3]
                normals = self[:, 3:]
            # from utils import mknn, device_mknn
            # knn = mknn(torch.cat([mid_points, normals], dim=-1).transpose(0, 1).unsqueeze(0), pc.transpose(0, 1).unsqueeze(0),
            #            args, self.device, mknn=n_mknn).to(device)
            ...
            # return args_min, valid projection mask
            ...
            # pc_per_face[knn_mask, :] = torch.cat([mid_points[knn_mask, :], self.normals[knn_mask, :].to(mid_points.device)], dim=-1)
